...
image_generator = ImageGenerator(size=15, centered=True, noise_rate=0.02)

print('Generating images')
images_train, images_val, images_test = image_generator.generate_image_sets(2000, flatten=True,
                                                                            share_train=0.7, share_validate=0.05)
...
train_loss_history, val_loss_history = model.fit(train_set, train_targets, val_set, val_targets,
                                                 batch_size=64, epochs=8)  # default batch size = 32
